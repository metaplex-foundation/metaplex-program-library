...
test('mint', async (t) => {
    ...
    const { txs } = await HELPER.addConfigLines(t, candyMachine.publicKey, payerPair, lines, fstTxHandler);
    // confirms that all lines have been written
    for (const tx of txs) {
        await fstTxHandler.sendAndConfirmTransaction(
            tx,
            [payerPair],
            'tx: AddConfigLines'
        ).assertNone();
    ...
    const { tx: wrapTx } = await API.wrap(t, address, candyMachine.publicKey, payerPair, fstTxHandler);
