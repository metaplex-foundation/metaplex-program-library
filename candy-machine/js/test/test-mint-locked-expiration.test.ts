...
const RELEASE_TIMESTAMP = new BN(Date.now() / 1000 + 5);
...
test('Candy machine initialize with lockup settings', async (t) => {
  ...
  t.assert(lockupSettings.lockupType === LockupType.ExpirationUnixTimstamp);
  t.assert(Number(lockupSettings.number) === RELEASE_TIMESTAMP.toNumber());
});
...
test('Mint with lockup', async (t) => {
  ...
  t.assert(timeInvalidator.parsed.maxExpiration.toNumber() === RELEASE_TIMESTAMP.toNumber());
});
