...
/// Accounts for the [`execute_sale_permissionless` handler](auction_house/fn.execute_sale_permissionless.html).
#[derive(Accounts)]
#[instruction(
    escrow_payment_bump: u8,
    free_trade_state_bump: u8,
...
pub struct ExecuteSalePermissionless<'info> {
    /// CHECK: Validated in execute_sale_logic.
    /// Buyer user wallet account.
    #[account(mut)]
    pub buyer: UncheckedAccount<'info>,
...
/// Execute sale between provided buyer and seller trade state accounts transferring funds to seller wallet and token to buyer wallet.
#[inline(never)]
fn execute_sale_permissionless_logic<'info>(
    ctx: Context<'_, '_, '_, 'info, ExecuteSalePermissionless<'info>>,
    escrow_payment_bump: u8,
    _free_trade_state_bump: u8,
    program_as_signer_bump: u8,
