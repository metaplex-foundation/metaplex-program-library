...
mod tests {
    ...
    use crate::{
        error::MetadataError,
        state::{EditionMarkerV2, Key, Metadata, TokenMetadataAccount},
        ID,
    };

    #[test]
    fn successfully_deserialize() {
        let expected_data = EditionMarkerV2::default();

        let mut buf = Vec::new();
        expected_data.serialize(&mut buf).unwrap();
        EditionMarkerV2::pad_length(&mut buf).unwrap();
        ...
        let data = EditionMarkerV2::from_account_info(&account_info).unwrap();
        assert_eq!(data.key, Key::EditionMarkerV2);
    ...
    fn deserializing_wrong_account_type_fails() {
        ...
        let error = EditionMarkerV2::from_account_info(&account_info).unwrap_err();
