...
impl TokenOwnedEscrow {
    pub fn len(&self) -> usize {
        let mut len = std::mem::size_of::<Key>();
        len += std::mem::size_of::<Pubkey>();
        len += 4 + self.tokens.len() * std::mem::size_of::<Option<Pubkey>>();
        len += 4 + self.delegates.len() * std::mem::size_of::<Pubkey>();
        len += std::mem::size_of::<Option<Pubkey>>();
        len
    }
...
impl EscrowConstraint {
    pub fn try_len(&self) -> Result<usize, ProgramError> {
        ...
        self.constraint_type.try_len().map(|ct_len| {
            Ok(ct_len + self.name.len() + std::mem::size_of::<u64>() + unknown_overhead)
        })?
    }
}
...
impl EscrowConstraintType {
    pub fn try_len(&self) -> Result<usize, ProgramError> {
        match self {
            ...
            EscrowConstraintType::Collection(_) => Ok(1 + std::mem::size_of::<Pubkey>()),
            EscrowConstraintType::Tokens(hm) => {
                if let Some(len) = hm.len().checked_mul(std::mem::size_of::<Pubkey>() + 1) {
