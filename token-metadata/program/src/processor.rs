...
pub fn process_instruction<'a>(
    ...
    match instruction {
        ...
        MetadataInstruction::ResetV2Metadata => {
            msg!("Instruction: Reset V2 Metadata");
            process_reset_v2_metadata(program_id, accounts)
        }
    }
}
...
pub fn process_reset_v2_metadata(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    let account_info_iter = &mut accounts.iter();
    let metadata_account_info = next_account_info(account_info_iter)?;
    let update_authority_info = next_account_info(account_info_iter)?;
    let metadata = MetadataV1::from_account_info(metadata_account_info)?;
    ...
    metadata.serialize(&mut m_v1)?;
    let v2_start = m_v1.len();
    let v2_end = metadata_account_info.data_len();

    // Clear all data after edition_nonce field
    let mut metadata_account_info_data = metadata_account_info.try_borrow_mut_data()?;
    metadata_account_info_data[v2_start..v2_end].fill(0);
